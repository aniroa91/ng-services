  ...
  def getJsonChurn() = withAuth {username => implicit request: Request[AnyContent] =>
    try{
      ...
      val churn2 = Json.obj(
        ...
        "churnPercent" -> rs.trendCallIn.map(x=> x._3 -> x._4)
      )
      val churn3 = Json.obj(
      ...
      val isNull6  = if(rs.callRegionAge.length > 0 && rs.callRegionAge.map(x=>x._4).min == 0 && rs.callRegionAge.map(x=> x._3).min == 0) 1 else 0
      val churn6 = Json.obj(
      ...
      val churn7 = Json.obj(
        ...
        "churnPercent" -> rs.churnCates.map(x=> x._3->x._4)
      )
      val json = Json.obj(
        "churn1" -> churn1,
        "churn2" -> churn2,
