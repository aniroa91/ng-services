...
class ChurnRegionController @Inject() (cc: ControllerComponents) extends AbstractController(cc) with Secured{
  ...
  def getJsonChurn() = withAuth {username => implicit request: Request[AnyContent] =>
    try{
      ...
      val isNull3   = if(rs.trendRegionMonth._2.length > 0 && rs.trendRegionMonth._2.map(x=>x._4).min == 0 && rs.trendRegionMonth._2.map(x=>x._3).min == 0) 1 else 0
      val churn3 = Json.obj(
      ...
      val isNull4  = if(rs.trendProfileMonth._3.length > 0 && rs.trendProfileMonth._3.map(x=>x._4).min == 0 && rs.trendProfileMonth._3.map(x=>x._3).min == 0) 1 else 0
      val churn4 = Json.obj(
      ...
      val isNull5 = if(rs.trendAgeProfile._2.length > 0 && rs.trendAgeProfile._2.map(x=>x._4).min == 0 && rs.trendAgeProfile._2.map(x=>x._3).min == 0) 1 else 0
      val churn5 = Json.obj(
